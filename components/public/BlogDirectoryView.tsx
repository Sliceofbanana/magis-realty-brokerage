...
export function BlogDirectoryView({ posts }: { posts: BlogPost[] }) {
  ...
  const topics = [...new Set(posts.flatMap((p) => p.tags))].slice(0, 6);
  ...
  if (!featured) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-24 text-center sm:px-6 lg:px-8">
        <h1 className="font-serif text-3xl font-bold text-navy-900">No articles yet.</h1>
        <p className="mt-2 text-sm text-gray-500">Check back soon for new insights.</p>
      </div>
    );
  }
